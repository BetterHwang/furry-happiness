 ...
 * 9.15 开启音频自定义渲染
 *
 * @param enable 指定是否启用音频自定义渲染，默认为关闭状态
 * @note 音频自定义渲染和 SDK 内部渲染两者互斥，需在进房前设置好，暂不支持进房后中途切换。
 */
- (void)enableCustomAudioRendering:(BOOL)enable;

/**
 * 9.16 开启音频自定义渲染时，向 SDK 获取音频数据
 *
 * TRTCAudioFrame 推荐如下填写方式：
 * - sampleRate：采样率，支持：16000、24000、32000、44100、48000。
 * - channel：声道数（如果是立体声，数据是交叉的），单声道：1； 双声道：2。
 * - data：分配音频帧 buffer。获取的 PCM 数据支持 10ms 或 20ms 帧长，推荐使用 20ms 帧长，【48000采样率、单声道的帧长度：
 * 48000 × 0.02s × 1 × 16bit = 15360bit = 1920字节】。
 *
 * @param audioFrame 音频数据
 * @note audioFrame 中的 sampleRate、channel 需提前设置好，同时分配好所需读取帧长的 data 空间。SDK 内部会根据 sampleRate、
 * channel 参数进行自动填充 data 数据。建议使用系统音频框架播放线程来定时调用此函数，以驱动音频数据渲染。
 */
